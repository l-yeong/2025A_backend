...
public class Example1 {
    public static void main(String[] args) {
        /*
            [ 예외 처리 ]
                1. 시스템 오류: 시스템이 종료되는 심각한 수 준의 문제
                ...
                6. 사용법
                    ...
                    catch( 예외 클래스명 매개변수명 ) { 지정한예외발생시 처리할 코드 }
                    catch( 예외 클래스명 매개변수명 ) { 지정한예외발생시 처리할 코드 }
                    catch( 예외 클래스명 매개변수명 ) { 지정한예외발생시 처리할 코드 }
        ...
        */
        ...
        // [1] 일반 예외: 예외가 있던/없던 무조건 예외처리 해야 한다.
        try { // 일반예외 발생 코드를 try{} 감싼다
            Class.forName("java.lang.String"); //Class.forName(클래스); // 해당 경로의 클래스를 읽어오는 함수
        ...
        //[2] 일반 예외, 예외발생
        try {
            Class.forName("java.lang.String2"); //String2 클래스는 존재 하지 않아서 예외 발생한다.
        }catch (ClassNotFoundException e){
            System.out.println("[예외발생2] String2 클래스가 없습니다"+e);
        } // [예외발생] String2 클래스가 없습니다java.lang.ClassNotFoundException: java.lang.String2
        ...
        try {
            ...
            int int3=Integer.parseInt(str3); //Integer.parseInt(); // 문자열 타입을 int 타입으로 반환하여
            int int4=Integer.parseInt(str4); //"100" --> 100, "1aa" -> 숫자가 될수 없다.
        }catch (NumberFormatException e){
            System.out.println("[예외발생5] 정수로 변환 불가능"+e);
        }

        // [6] 실해예외:ArrayIndexOutOfBoundsException : 존재 하지않는 인덱스 호출 예외
        try{
            int[] intArray={1,2,3};
            System.out.println(intArray[0]); // 0번 인덱스 존재한다.
            System.out.println(intArray[4]); // 4번 인덱스 존재하지 않는다. 오류 발생
        }catch (ArrayIndexOutOfBoundsException e){
            System.out.println("[예외발생6] 없는 인덱스 입니다"+e);
        }

        // [7] 실행예외: InputMismatchException : 타입이 일치 하지 않을때 예외
        try {
            Scanner scan = new Scanner(System.in);
            System.out.println("정수입력: ");
            int value7 = scan.nextInt();
        }catch (InputMismatchException e){
            System.out.println("[예외발생7] 입력한 타입이 불일치"+e);
        }
        ...
        try {
            ...
            System.out.println("다중캐치"+e1);
        }catch (ArrayIndexOutOfBoundsException e2){
            System.out.println("다중캐치"+e2);
        }catch (NullPointerException e3){
            System.out.println("다중캐치"+e3);
        }catch (Exception e4){
            System.out.println(e4);
        }
        // -주의할점1: 다중 catch시 먼저 발생한 예외 부터 하나의 예외 발생 하고 하나의 catch만 실행 된다
        // -주의할점2: Exception 클래스는 모든 예외를 처리 하므로 다중 catch 시 예는 맨 아래에서 정의 한다.
        finally {
            System.out.println("예외가 있던 없던 무조건 실행 코드");
        }
