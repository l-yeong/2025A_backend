...
class D{
    ...
    //static
    static int 비멤버변수; //객체내 포함하지 않고 클래스가 (new) 하지 않아도 사용 가능
...
public class Example2 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(D.비멤버변수); // 객체 없이 static 가능
        D.비멤버변수=20;
        System.out.println(d.비멤버변수); // 객체도 static 가능

        //[3] static final 상수 호출, 클래스명.상수명;
        System.out.println(D.상수);

        //[4] static 메소드 내 static 유무 호출
        //멤버메소드();//불가능;
    ...
    [ static ] * 정적인
        1. 프로그램이 실행될때 메모리가 할당되고 프로그램이 종료 될때 메모리 삭제 되는 키워드
        2. 사용법: static 타입 멤버변수 -------> static변수
        3. 주의할점: 멤버변수(객체내변수) new vs static(우선할당)
    ...
    [ static 유무 중요한 이유 ]
        ...
        //방법1: 객체 생상하여 객체를 통해 static 아닌 메소드 호출한다.

        class Test{
            static void main3(){}
            void main2(){ }
            static void main(){
                main2(); // 불가능: main함수는 우선 할당이 되면서 main2가 누구세요?
                new Test().main2(); // 가능
                main3(); // 가능: main함수는 우선 할당이 되면서 우선할당된 main3 가 알고 있다
            }
        }test e
    */
    } //main e
