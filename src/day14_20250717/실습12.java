...
class Animal{
    void makeSound(){System.out.println("동물이 소리를 냅니다");//메소드
    }
}//animal end

class Cat extends Animal{
    void makeSound(){System.out.println("고양이가 야옹하고 웁니다");//메소드
    }
} //cat end

class Machine{
    Machine(){System.out.println("부모 클래스 생성자 실행");//생성자
    }
}//Machine end

class Computer extends Machine{
    Computer(){System.out.println("자식 클래스 생성자 실행");//생성자
    }
}//Computer end
...
class Shape{
    void draw(){System.out.println("도형을 그립니다");}//메소드
}// Shape end

class Circle extends Shape{
    void draw(){System.out.println("원을 그립니다");} //메소드
}//Circle end

class Vehicle{
} // Vehicle end
class Bus extends  Vehicle{
    void checkFare(){System.out.println("요금을 확인합니다.");}//메소드
} // bus end

class Beverage{
    void drink(){System.out.println("음료를 마십니다");} //메소드
}// Beverage end

class Coke extends Beverage{
    void drink(){System.out.println("콜라를 마십니다");} //메소드
}//Coke end
class Coffee extends Beverage{
    void drink(){System.out.println("커피를 마십니다");} //메소드
}//Coffee end

class Weapon{
    void attack(){System.out.println("무기로 공격합니다");} //메소드
}//Weapon end

class Character{
    void use(Weapon weapon){ weapon.attack();} //attack 호출 메소드
}//Character

class Sword extends Weapon{
    void attack(){System.out.println("검으로 공격합니다");} //메소드
}//Sword end
...
class SuperClass {
    //멤버변수
    String name="상위";
    void method(){System.out.println("상위 메소드");} //메소드
} // SuperClass end

class SubClass extends SuperClass{
    String name = "하위";
    void method(){System.out.println("하위 메소드");} //메소드
}// SubClass end
...
//
        Figure triangle = new Triangle(); // 상하관계 이면서 하위 클래스가 상위 클래스를 선택 할수 있기 때문인거같습니다.
...
//        3.main 함수에서 Shape shape = new Circle(); 코드를 작성한 뒤, shape.draw()를 호출했을 때 어떤 결과가 나오는지 확인하고 그 이유를 주석으로 설명하세요.
        ...
        shape.draw(); // "원을 그립니다" : 상위 타입 일뿐 태생은 Circle 이기때문에 "원이 그립니다" 호출 된다고 생각 합니다.
...
//        4. 만약 Bus 타입이 맞다면, Bus 타입으로 강제 변환한 뒤 checkFare() 메소드를 호출하세요.
        ...
        if(vehicle instanceof Bus){
            System.out.println("타입맞다");
        }else{
            System.out.println("타입 아니다");
        }
...
//        3. SuperClass obj = new SubClass(); 로 객체를 생성한 뒤, obj.name과 obj.method()를 각각 호출했을 때의 결과를 확인하고, 왜 다른 결과가 나오는지 주석으로 설명하세요.
        ...
        obj.method();   // Example1에 내용이 있긴한데 잘 모르겠습니다..
//
//[문제 10] 다중 상속 관계
//        1. Device 클래스를 만드세요.
