...
select mid as 회원아이디, mname 회원명 from member;	-- as는 생략 하고 띄워쓰기로 구분
...
#[4] 조건절 
#비교연산자: >초과 <이만 >=이상 <=이하 =같다 !=같지않다.
select *from member where mname='블랙핑크';		-- member 테이블의 manme속성의 값이 '블랙핑크'와 같으면
select *from member where mnumber=4;			-- member 테이블의 mnumber속성의 값이 4 이면
...
select *from member where mheight <=162;		-- member 테이블의 mheight속성의 값이 162 이하인
select *from member where mheight >=165 and mheight <= 170;			-- mheight속성의 값이 165이상 이면서 170 이하
select *from member where mheight between 165 and 175; 				-- mheight속성의 값이 165이상 이면서 170 이하
...
#기타연산자: between 시작값 and 끝값

select *from member where mphone1=' ';			-- 속성값이 비어있는 표현, ' '빈칸 대신에 null 사용된다.
select *from member where mphone1=null;			-- 속성값이 null 이면[불가능]
select *from member where mphone1 is null;		-- 속성값이 null 이면[가능]
...
select *from member where mname like '%핑크';		-- 속성값이 '핑크'로 끝나는 , x핑크, xx핑크, xxx핑크 ~
select *from member where mname like '%이%';		-- 속성값에 '이'가 포함된
select *from member where mname like '_이_';		-- 속성값에  두번째 글자가 '이'인 세글자
...
# 주의할점 2개 이상의 정렬은, (쉼표)구분 하되, 1차정렬에서 중복값이 있을경우 중복값 끼리 2차정렬
# (1차) 먼저 주소(maddr) 내림차순으로 정렬후 (2차) 주소가 동일한 데이터끼리 mdebut(날짜) 오른차순 정렬host_summary_by_file_iohost_summary_by_file_io
