...
# 예1] 방문록을 기록하는 데이터 설계
create table board(
	content longtext, 	#게시물의 내용	 # 자바= priavte String content
    writer varchar(30)	#게시물의 작성자 # 자바= private String writer
);
...
create table waiting(
	phone char(13),		# 핸드폰 번호 # 고정길이 13글자 # 자바=private String phone
    count tinyint		# 인원수 # 최대 100정도 #자바=private byte count
);
...
create table test4(
	# 필드명 타입 제약조건
    필드명1 tinyint not null	,  			-- 지정한 속성에는 null를 대입할수 없도록 조건 설정.
    필드명2 smallint unique	,  			-- 지정한 속성에는 중복값을 저장 할수 없도록 조건 설정
    필드명3 int default 10	,  			-- 지정한 속성에는 값이 생략될때 자동으로 10 기본값으로 설정
...
create table test5(
    ...
    -- test5의 '필드명1' 속성을 FK필드로 설정 하고 test4의 '필드명4' 속성의 PK를 참조 한다.
);
...
create table member(						-- 회원테이블
	mno int auto_increment,					-- 회원번호: 정수, 자동번호, PK사용할예정
...
create table board(							-- [2] 게시물 테이블
    ...
    btitle varchar(100) not null,			-- 게시물제목: 문자최대 100, 공백x
    ...
    mno int ,								-- 게시물 작성자: FK 사용할 예정 ---> PK필드 타입과 일치
    constraint foreign key (mno) references member(mno)
);

# 여럿 테이블 간의 관계 확인(다이어그램) : workbench 상단메뉴->[database] -> 
select *from board;
