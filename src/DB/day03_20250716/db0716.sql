...
# [1] INSERT : 레코드 삽입, "레코드 란? 테이블내 행(가로) 단위 의 데이터들, 1레코드 vs 1행 vs 1줄
# 주의할점: 테이블내 데이터 삽입시 레코드 단위, 한칸한칸에 데이터삽입이 아닌 줄/행/레코드 단위 삽입
# (1) insert into 테이블명(속성명1,속성명2) valuse(값1,값2);
insert into test(mno, mname, mcount)values (1, "유재석",30);
# 작성한 속성명에 대응하는 값 삽입한다.
insert into test(mcount,mno,mname) values (40, 2, "강호동");
# (2) auto_increment 제약조건의 속명 값 생략할 경우 자동번호(1씩증가된)할당 
insert into test(mcount,mname) value(50,"신동엽");
# (3) default 제약조건의 속성은 값 생략할경우 정의된 기본값이 할당			
insert into test(name) value("신동엽");
...
# (7) 하나의 insert 에서 여러개 레코드 삽입할경우, valuse( ),( ),( );
insert into test values(6,"박명수",50),(7,"박명수2",60);
select *from test; #확인차

# [2] SELECT 조회              
# 주의할점: 조회 결과는 기존 테이블과 상관 없다. select만 result table이 존재한다. 조회결과는 항상 테이블/표
# (1) select *from 테이블명: 전체속성(*)의 레코드 조회 
select *from test;
...
# 'test' 테이블에서 'mname'속성값이 '유재석'인 레코드를 조회
select *from test where mname= "유재석";# =(대입이 아닌 같다라는뜻)
select *from test where mno =3; #'mno' 속성 값이 3과 같은 레코드 조회

# [3] UPDATE: 레코드 수정
# mysql workhench 에서는 수정/삭제를 safe mode 이므로 불가능
...
# 3. update 테이블명 set 속성명1 = 값1, 속석명2 = 값2; where 조건절;	: 다수의 속성값 수정
update test set mcount = 20, mname="신동엽2" where mno=2;
