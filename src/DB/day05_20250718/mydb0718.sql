...
select count(*) from buy;		# count(*) : (레코드)개수, null포함
...
# 2. 회원아이디별 총구매금액 이 1000초과 인 레코드 조회
...
# [*] select 키워드 간의 작성 규칙
# selct 속성명 from 테이블명 where 일반조건 group by 그룹속성명 having 그룹조건 order by 정렬속성명 asc/desc limit개수;
#[*] 처리 순서
