...
select avg(price) as 평균단가 from books 평균단가;

-- [문제 3] orders 테이블에서 '김민준' 고객이 주문한 총 주문 수량(order_qty의 합계)을 조회하세요.
select customer,sum(order_qty) as 주문수량 from orders where customer="김민준" group by customer having 주문수량;
...
select customer,sum(order_qty) as 합계 from orders where customer in("김민준","이서연")  group by customer having 합계 >=4;

-- [문제 7] books 테이블에서 재고(stock)가 5권 미만인 도서들의 평균 가격을 조회하세요.
select avg(price) from books where stock < 5 having avg(price); 
...
select customer, sum(order_qty) as 총주문 from orders group by customer having 총주문 >=5;

-- [문제 10] books 테이블에서 각 저자(author)별로 출판한 도서가 2권 이상인 저자의 이름과 출판 도서 수를 조회하세요.
select *from books;select author, count(*) as 도서수 from books group by author having 도서수 >=2;
...
select count(genre) from books where genre="소설";

-- [문제 13] orders 테이블에서 주문을 한 고객이 총 몇 명인지 조회하세요. (DISTINCT 사용)
select count(distinct book_id) from orders;

-- [문제 14] orders 테이블에서 고객별로 첫 주문일과 마지막 주문일을 조회하세요.
select  customer, order_date from orders order by order_date asc limit 0,5;
