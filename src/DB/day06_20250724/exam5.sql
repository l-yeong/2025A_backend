...
# JOIN 필요한 이유 : 제품명과 카테고리명은 서로 다른 테이블에 존재 한다. 즉] 서로 다른 테이블을 하나로 집합 하기
# !! JOIN 조건: 서로 다른 테이블간의 동일한 속성값이 필요하다. 주로] PK와 FK 속성이 주로 같은 값을 같는다.
...
select *from product p inner join pcategory pc on p.카테고리번호_fk = pc.카테고리번호_pk;#3. 교집합(두 집합[테이블]간의 공통)
#4. 교집합 이후 원하는 속성명 추출
...
select p.제품명, sk.재고수량 from product p inner join stock sk on p.제품번호_pk = sk.제품번호_fk;

-- [문제 4] '그램 15인치' 제품의 모든 재고등록날짜와 재고수량을 조회하세요. (조회결과: 2개 레코드)
select p.제품명, sk.재고등록날짜,sk.재고수량 from product p inner join stock sk on p.제품번호_pk = sk.제품번호_fk where p.제품명='그램 15인치';
-- [문제 5] 모든 제품의 제품명, 카테고리명, 재고수량을 한 번에 조회하세요. (3개 테이블 조인) (조회결과: 8개 레코드)
select *from product;
select *from pcategory;
select *from stock;
select p.제품명, pc.카테고리명 ,sk.재고수량 from product p 
	inner join pcategory pc on pc.카테고리번호_pk = p.카테고리번호_fk 
	inner join stock sk on p.제품번호_pk = sk.제품번호_fk;
-- [문제 6] 모든 카테고리의 카테고리명과 해당 카테고리에 속한 제품명을 조회하세요. 만약 카테고리에 속한 제품이 없더라도 카테고리명은 모두 표시되도록 하세요. (조회결과: 7개 레코드)
select p.제품명 from pcategory pc left outer join product p on pc.카테고리번호_pk = p.카테고리번호_fk;

-- [문제 7] 재고가 한 번도 등록되지 않은 제품의 제품명을 조회하세요. (조회결과: 1개 레코드)
select p.제품명 from product p left join stock sk on p.제품번호_pk = sk.제품번호_fk where sk.제품번호_fk is null;

-- [문제 8] 각 카테고리별로 총 재고 수량의 합계를 카테고리명과 함께 조회하세요. (조회결과: 3개 레코드)
select *from pcategory;
select *from stock;
select pc.카테고리명, sum(sk.재고수량) from pcategory pc inner join product p on pc.카테고리번호_pk = p.카테고리번호_fk inner join stock sk on p.제품번호_pk = sk.제품번호_fk group by pc.카테고리명;

-- [문제 9] 각 제품별로 총 재고 수량을 조회하고, 총 재고 수량이 많은 순서대로 정렬하여 제품명과 총재고수량을 표시하세요. (조회결과: 6개 레코드)
select *from product;
select p.제품명, sum(sk.재고수량) 총수량 from product p inner join stock sk on p.제품번호_pk = sk.제품번호_fk group by p.제품명 order by 총수량 desc;