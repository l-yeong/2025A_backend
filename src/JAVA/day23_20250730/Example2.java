    ...
    public static void main(String[] args) {
        /*
            [ 컬렉션 프레임쿼크2 ]
                ...
                Set 인터페이스:  집합구조, 인덱스 없다. 중복허용하지않는다. (자료가 식별자) + 중복없는 집합
        */
        // [1] HashSet 객체 생성
        Set<String> set1 = new HashSet<>();
        // [2] 주요 메소드
            // 1) .add(자료) : 집합 내 지정한 자료를 추가 한다. 인덱스(순서) 없음
        set1.add("유재석"); set1.add("강호동"); set1.add("신동엽"); set1.add("유재석"); //이미 존재하는 자료는 중복이므로 불가능
        System.out.println(set1); // [유재석,강호동,신동엽]
            // 2) .size() : 집합 내 자료/요소 개수 변환
        ...
        set1.forEach((str)->{System.out.println("forEach "+str);});

        // 4) 구현체
        // set 인터페이스: 순서(인덱스) 보장 없이 요소들을 저장 하는 구조

        // 1) HashSet: 중복없는 여러개 요소 저장 + 빠르다
        HashSet<Integer> hashSet = new HashSet<>();
        // 2) TreeSet: 중복없는 여러개 요소 저장 + 저장한 자료들을 자동으로 정렬(오름차순)
        TreeSet<Integer> treeSet = new TreeSet<>();
            treeSet.add(20); treeSet.add(10); System.out.println(treeSet);
