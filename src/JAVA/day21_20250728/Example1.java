    ...
    public static void main(String[] args) {
        ...
        [ 스레드  Thread ]
            ...
            4. 사용법
                ...
                - public static void main(String[] args){}
        [ 멀티스레드 Multi-Thread ]
            1. 정의: main스레드 외 새로운 작업스레드를 생성하여 동시 작업
            2. 목적: *벙렬처리*, 동시에 여러 작업을 수행
            3. 사용처: 웹/앱, 채팅/첨부파일/JDBC, 동기화/비동기화
            4. 사용법:
            - 단일(싱글): 1차선 도로
            - 병렬(멀티): 다차선 도로
                (1) 익명 구현체
                    Thread thread = new Thread(new Runanble(){@Override public void run(){
                            //작업 스레드가 처리할 코드
                        }
                    });
                    thread.start();
                ...
                (2)구현체
                    class 작업클래스 implement Runnable{
                        @Override public void run(){
                            //작업스레드가 처리할 코드
                        }
                    };
                    --------------------------------------
                    Thread thread = Thread(new 작업클래스());
                    thread.start();
            ...
            5. 주요 클래스/인터페이스
                ...
                (2) Runnable 인터페이스
                    1) run 추상메소드: 작업 스레드가 실행하는 최초 시작점 처리 메소드
        ...
        */
        ...
        for(int i=1; i<=5; i++){
            ...
            try {Thread.sleep(1000); //Thread.sleep(밀리초): 현재 스레드를 밀리초 만큼 일시정지+예외처리
            }catch (Exception e){System.out.println(e);}//catch end
        }//for end


        //[2] 멀티 스레드 예: 익명 구현체: 인터페이스타입을 클래스 없이 직접 구현
        //1) 출력 5번
        System.out.println("[1] MAIN 스레드가 읽어드리는 코드"); //main 메소드가 처리하는 코드
        for(int i=1; i<=5; i++){
            System.out.println("[1] MAIN 스레드가 읽어드리는 코드"+i);
        ...
        Thread thread1 = new Thread(runnable1); // run 구현한 runnable 타입을 Thread 생성자 대입
        thread1.start(); //run 구현된 runnable 타입을 갖는 thread 객체가 작업 스레드 .start() 실행

        // [3] 멀티 스레드 예2: 구현체
        작업스레드1 작업스레드1 = new 작업스레드1();
        Thread thread2 = new Thread(작업스레드1);
        thread2.start();
