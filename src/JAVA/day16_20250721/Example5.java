...
public class Example5 {
    public static void main(String[] args) {
        ...
        try { //(4) 모든 코드를 감싼 예외 처리
            String path = "./src/day16_20250721/인천광역시 부평구_맛있는 집(맛집) 현황_20240801.csv";
            File file = new File(path);
            System.out.println(file.exists()); // (1) 파일 존재 여부 확인

            if( file.exists()){ // (2) 존재하면 파일 읽어오기
                FileInputStream fin = new FileInputStream(path); // (3) 파일 입력 객체 생성
                byte[] bytes = new byte[(int)file.length()]; // (5) 읽어온 바이트를 저장할 바이트 배열 용량 만큼 선언
                ...
                System.out.println(str.split("\n")[1].split(",")[1]); //두번째 행의 두번째 열: (주)온누리푸드온누리장작구이
                //---------------------------------------------------------------------
                ...
                for(int i=0; i < 행데이터.length; i++){
                    ...
                    //System.out.println(열데이터[3]); // 지점메뉴
