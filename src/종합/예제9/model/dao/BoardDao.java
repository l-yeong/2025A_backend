...
public class BoardDao {
    ...
    public ArrayList<BoardDto> boardPrint(){
        //ArrayList<BoardDto> : 반환타입으로 모든 dto 반환한다
    ...
    public void loadCSV() {
        try {
            ...
            for (String[] row : inData) { // 향상된 for문 (4) 반복문이
                String content = row[0]; //첫번째열
                String writer = row[1]; //두번째열
                BoardDto boardDto = new BoardDto(content, writer); //(5) 각 열들을 하나의 dto로 생성
                boardDB.add(boardDto); // (6)생성은 dto를 리스트에 저장
            }//for end
